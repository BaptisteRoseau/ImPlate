 ...
 * @param blurred the matrix object buffer for the blurred picture
 ...
 */
void blur_pixel(cv::Mat picture, cv::Mat blurred, unsigned int filter_size, int i, int j, int k);
 ...
 */
int blur(const cv::Mat picture, cv::Mat blurred, const std::vector<cv::Point> &corners,
